...
public struct ClipboardCapture: Sendable, Equatable {
    ...
    /// Markers apps use to say "do not put this in clipboard history".
    /// Password managers set the concealed type; utilities that stage a value
    /// for one paste set the transient one. See nspasteboard.org.
    static let excludedTypes: [NSPasteboard.PasteboardType] = [
        NSPasteboard.PasteboardType("org.nspasteboard.ConcealedType"),
        NSPasteboard.PasteboardType("org.nspasteboard.TransientType"),
        NSPasteboard.PasteboardType("org.nspasteboard.AutoGeneratedType"),
    ]

    /// True when the source app asked for this clip not to be recorded.
    public static func isExcluded(_ pasteboard: NSPasteboard) -> Bool {
        guard let types = pasteboard.types else { return false }
        return excludedTypes.contains(where: types.contains)
    }
    ...
    ///
    /// - Parameter honourExclusionMarkers: when true, clips an app marked as a
    ///   password or as transient are skipped entirely.
    public static func read(
        from pasteboard: NSPasteboard,
        honourExclusionMarkers: Bool = true
    ) -> ClipboardCapture? {
        if honourExclusionMarkers, isExcluded(pasteboard) { return nil }
