...
struct ClipboardCaptureTests {
    ...
    @Test("Clips marked concealed by a password manager are skipped")
    func concealedClip() {
        let board = pasteboard()
        ...
        #expect(ClipboardCapture.read(from: board) == nil)
        // The user can opt back into capturing everything.
        #expect(ClipboardCapture.read(from: board, honourExclusionMarkers: false)?.preview == "hunter2")
    }

    @Test("Transient and auto-generated markers are honoured too", arguments: [
        "org.nspasteboard.TransientType",
        "org.nspasteboard.AutoGeneratedType",
    ])
    func otherExclusionMarkers(marker: String) {
        let board = pasteboard()
